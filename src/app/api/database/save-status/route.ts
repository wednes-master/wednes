...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        counts: {
          notices: (noticesCount[0] as { count: number } | undefined)?.count || 0,
          events: (eventsCount[0] as { count: number } | undefined)?.count || 0,
          updates: (updatesCount[0] as { count: number } | undefined)?.count || 0,
          gameContents: (gameContentsCount[0] as { count: number } | undefined)?.count || 0,
          alarms: (alarmsCount[0] as { count: number } | undefined)?.count || 0,
          marketItems: (marketItemsCount[0] as { count: number } | undefined)?.count || 0,
          auctionItems: (auctionItemsCount[0] as { count: number } | undefined)?.count || 0,
          marketItemDetails: (marketItemDetailsCount[0] as { count: number } | undefined)?.count || 0,
          apiLogs: (apiLogsCount[0] as { count: number } | undefined)?.count || 0,
        },
        recent: {
